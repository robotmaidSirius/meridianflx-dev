...
def create_keyfile(sub_dir, root_dir="."):
    ...
    with open(file_path, "w", encoding='utf-8') as file:
        ...
        file.write('\n/* Wifiアクセスポイントの設定 */\n')
        file.write('#define WIFI_AP_SSID        "' + WIFI_AP_SSID       + '"   /*! アクセスポイントのSSID */\n')
        file.write('#define WIFI_AP_PASS        "' + WIFI_AP_PASS       + '"   /*! アクセスポイントのパスワード */\n')
        file.write('#define UDP_SEND_IP         "' + UDP_SEND_IP        + '"   /*! 送り先のPCのIPアドレス (PCのIPアドレスを調べておく) */\n')
        ...
        file.write('#define HOST_NAME           "' + HOST_NAME          + '"   /*! このボードのホスト名 */\n')
