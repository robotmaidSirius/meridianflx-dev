 ...
 * @note MIT LICENSE
 */
#ifndef __MERIDIAN_MODULE_MRD_WIFI_HPP__
#define __MERIDIAN_MODULE_MRD_WIFI_HPP__
// ヘッダファイルの読み込み
...
bool mrd_wifi_init(WiFiUDP &a_udp, const char *a_ssid, const char *a_pass, int port,
...
bool mrd_wifi_udp_send(byte *a_meridim_bval, int a_len,
                       IPAddress ip_address, uint16_t port,
                       WiFiUDP &a_udp) {
  a_udp.beginPacket(ip_address, port); // UDPパケットの開始
  a_udp.write(a_meridim_bval, a_len);  // データの書き込み
  a_udp.endPacket();                   // UDPパケットの終了
  return true;
}

#endif // __MERIDIAN_MODULE_MRD_WIFI_HPP__
