...
void mrd_wire0_read_ahrs_i2c(bool imuahrs_available) // ※wireTimer0.beginの引数のためvoid必須
{
...
{
  mrd_wire0_read_ahrs_i2c(flg.imuahrs_available);
}
...
bool mrd_wire0_intervaltimer_start(bool a_wr_check) {
  if (a_wr_check) {
    // wireTimer.priority(90); // 優先度を設定する場合はここで指定
...
bool mrd_meriput90_ahrs(Meridim90Union &a_meridim, float a_ahrs_result[], bool &imuahrs_available) {
